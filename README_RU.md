...
**[Текущая версия: 1.8.5](https://raw.githubusercontent.com/rigljs/rigl/main/rigl.min.js)**
...
Если всё-таки понадобится создать закрытый компонент, то необходимо будет перенести *Маршрутизатор* в него, а элемент содержащий ссылки, назначить вместо объекта *Document* по умолчанию:
...
<r-menu closed>
  ...
  <script>
    ...
    // определить маршрут "/"
    router.on('/', () => {
      // присвоить свойству "path" строку
      this.path = 'r-home'

      /* запустить событие "change-page" с передачей ключевого слова "this"
        в свойство "currentTarget" объекта Event */
      obs.trigger('change-page', this)
    })

    // определить маршрут "/about"
    router.on('/about', () => {
      // присвоить свойству "path" объект
      this.path = {
        // название компонента
        component: 'r-about',
        // атрибуты компонента
...
<r-content>
  ...
  <script>
    ...
    /* определить новое событие "change-page" и функцию обратного вызова, в которой значение свойства "path"
      из компонента R-MENU, присваивается свойству "page" из компонента R-CONTENT */
