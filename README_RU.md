...
В Rigl существуют три вида циклов, которые задаются с помощью специального атрибута ***$for***. Содержимое элемента-контейнера в котором указывается этот атрибут будет повторяться столько раз, сколько итераций необходимо для полного перебора цикла.

Цикл *for-of* используется для перебора итерируемых объектов, например массивов: 
